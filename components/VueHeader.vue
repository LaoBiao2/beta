	...
	<div class="VueHeader">
		<div class="content clearfix">
			...
			<ul>
				...
				<li v-for="(nav, key) in navList" :key="key" @mouseenter="hoverIn(key)" @mouseleave="hoverOut(key)">
                    <nuxt-link :to="nav.nav_url" class="first-a">
                        {{nav.nav_name}}
                        <i v-if="nav.child != ''"></i>
                    </nuxt-link>
                    <div class="sub-menu" v-if="nav.child != ''">
                        <i></i>
                        <nuxt-link :to="sub.nav_url" 
                                v-for="(sub, key2) in nav.child" 
                                :key="key2"
                        >{{sub.nav_name}}</nuxt-link>
                    </div>
                </li>
			</ul>
		</div>
...
import '~/static/animate.css';
	export default {
        ...
        methods: {
            hoverIn (index) {
                $(".VueHeader ul li").eq(index).find(".sub-menu").addClass("animated fadeInUp").removeClass("fadeOutDown");
            },
            hoverOut (index) {
                $(".VueHeader ul li").eq(index).find(".sub-menu").addClass("animated fadeOutDown").removeClass("fadeInUp");
            }
        }
	};
